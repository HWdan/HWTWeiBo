...
public class HYLabel: UILabel {
    ...
    // 懒加载属性
    lazy var textStorage : NSTextStorage = NSTextStorage() // NSMutableAttributeString的子类
    lazy var layoutManager : NSLayoutManager = NSLayoutManager() // 布局管理者
    lazy var textContainer : NSTextContainer = NSTextContainer() // 容器,需要设置容器的大小
    
    // 用于记录下标值
    lazy var linkRanges : [NSRange] = [NSRange]()
    lazy var userRanges : [NSRange] = [NSRange]()
    lazy var topicRanges : [NSRange] = [NSRange]()
    ...
    // 闭包属性,用于回调
    var tapHandlerType : TapHandlerType = TapHandlerType.noneHandle
...
extension HYLabel {
    ...
    func getRangesFromResult(regex : NSRegularExpression) -> [NSRange] {
        // 1.匹配结果
...
extension HYLabel {
    ...
    func getSelectRange(selectedPoint : CGPoint) -> NSRange? {
